...
def _raise_if_non_finite(name, tensor, extra_context=None):
    if torch.isfinite(tensor).all():
        return
    message = f"Non-finite values detected in Mask2Former decoder. {_finite_tensor_summary(name, tensor)}"
    if extra_context:
...
class MultiScaleMaskedTransformerDecoderForOPTPreTrain(nn.Module):
    ...
    def forward_woconcat(self, x, mask_features, mask=None, seg_query=None, SEG_embedding=None):
        ...
        if self.use_seg_query:
            ...
            query_embed = self.new_query_embed.weight.unsqueeze(1).repeat(1, bs, 1)
    ...
    def forward_prediction_heads(self, output, mask_features, attn_mask_target_size, SEG_embedding=None,
        ...
        _raise_if_non_finite(
            ...
            outputs_mask,
            extra_context=(
                f"attn_mask_target_size={attn_mask_target_size}; "
                f"{_finite_tensor_summary('mask_embed', mask_embed)}; "
        ...
        attn_mask = F.interpolate(outputs_mask.float(), size=attn_mask_target_size, mode="bilinear",
                                  align_corners=False).to(mask_embed.dtype)
        _raise_if_non_finite(
            "attn_mask_before_bool",
            attn_mask,
            extra_context=f"attn_mask_target_size={attn_mask_target_size}",
        )
        # must use bool type
        # If a BoolTensor is provided, positions with ``True`` are not allowed to attend while ``False`` values will be unchanged.
        attn_mask = (attn_mask.sigmoid().flatten(2).unsqueeze(1).repeat(1, self.num_heads, 1, 1).flatten(0,
